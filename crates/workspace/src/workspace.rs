...
impl Workspace {
    ...
    pub fn actions(&self, div: Div, window: &mut Window, cx: &mut Context<Self>) -> Div {
        self.add_workspace_actions_listeners(div, window, cx)
            ...
            .on_action(cx.listener(Self::reopen_last_modal))
    ...
    fn reopen_last_modal(
        &mut self,
        _: &ReopenLastModal,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        // Defer: when this is triggered from within another modal (e.g. the command
        // palette), that modal's dismissal is processed asynchronously. Revealing now
        // would be a no-op because a modal is still active, so wait for it to close.
...
mod tests {
    ...
    /// A modal that registers its focus handle as a reopenable picker on
    /// construction, mirroring what a real `Picker` does. The modal layer
    /// recognizes it by focus identity and stashes it on dismissal.
    ...
    #[gpui::test]
    async fn test_reopen_last_modal(cx: &mut gpui::TestAppContext) {
        init_test(cx);
        ...
        // Opening (and closing) a non-reopenable modal must NOT discard the stash,
        // so the picker can still be revealed afterwards. This is what lets the
        ...
        // Reopen triggered from *within* another modal: that modal dismisses
        // asynchronously and dispatches the action in the same cycle (this is how
        // the command palette works). The deferred reveal must run once that modal
        // is gone.
        workspace.update_in(cx, |workspace, window, cx| {
            workspace
                .modal_layer
        ...
        workspace.update_in(cx, |workspace, window, cx| {
            ...
            workspace.reopen_last_modal(&ReopenLastModal, window, cx);
        });
        cx.executor().run_until_parked();
        assert!(
