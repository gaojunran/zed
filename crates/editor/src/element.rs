...
fn compute_size_by_content_layout(
    ...
    // Deterministically check whether a horizontal scrollbar will appear by
    // measuring the longest line's rendered width. This avoids the two-pass
    // approach (using stale last_position_map) that can flash a wrong frame.
    let settings = EditorSettings::get_global(cx);
    let horizontal_scrollbar_enabled = settings.scrollbar.show != ShowScrollbar::Never
        && settings.scrollbar.axes.horizontal
        && editor.show_scrollbars.horizontal
    ...
    if horizontal_scrollbar_enabled {
        ...
        let longest_line_width =
            layout_line(snapshot.longest_row(), &snapshot, style, editor_width, |_| false, window, cx)
                .width;
...
mod tests {
    ...
    use gpui::{TestAppContext, VisualTestContext, WindowHandle, font};
    ...
    #[gpui::test]
    async fn test_size_by_content_includes_horizontal_scrollbar_height(
        cx: &mut TestAppContext,
    ) {
        init_test(cx, |_| {});
        ...
        // Long line that overflows horizontally: height should include scrollbar width.
        ...
        // Short text that fits: height should NOT include scrollbar width.
        ...
        // Horizontal scrollbar disabled: height should NOT include scrollbar width
        // even with a long line.
